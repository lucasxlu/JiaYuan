    ...
    for each_json_file in os.listdir(lover_root_dir):
        with open(lover_root_dir + '/' + each_json_file, 'r', encoding='utf-8') as f:
            ...
            for each_line in f.readlines():
                repaired_each_line = each_line.replace('\'', '\"').replace('None', 'null').replace('\\u', '')
...
def write_excel(lists, filename):
    ...
    ws.cell(row=1, column=11).value = '用户级别'
    ws.cell(row=1, column=12).value = '择偶标准'
    ws.cell(row=1, column=13).value = '用户标签'
    ...
    for each_item in lists:
        ...
        for each_job_info_obj in info_list:
            try:
                ...
                ws.cell(row=rownum, column=11).value = BeautifulSoup(each_job_info_obj['userIcon'], 'html.parser').i[
                    'title']
                ws.cell(row=rownum, column=12).value = each_job_info_obj['matchCondition']
                ws.cell(row=rownum, column=13).value = BeautifulSoup(each_job_info_obj['randListTag'],
                                                                     'html.parser').find(
                    'span').get_text()
            except:
                pass
